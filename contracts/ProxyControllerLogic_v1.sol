...
contract ProxyControllerLogic_v1 is VersionLogic {
    ...
    function createOrganization(address _organizationsAddress, bytes32 _organizationKey, uint _nonce, bytes _clientSign) public onlyByVersionContractOrLogic {
        require(Organizations(_organizationsAddress).createOrganizationWithSign(_organizationKey, _nonce, _clientSign));
    }

    function addOrganizationMember(address _organizationsAddress, address _addr, uint _nonce, bytes _clientSign) public onlyByVersionContractOrLogic {
    ...
    function createPersonalInformation(address _personalInformationsAddress, address _user, bytes32 _dataKey, bytes32 _dataHash, uint _expires, uint _nonce, bytes _clientSign) public onlyByVersionContractOrLogic {
    ...
    function updatePersonalInformation(address _personalInformationsAddress, address _user, bytes32 _dataKey, bytes32 _dataHash, uint _expires, uint _nonce, bytes _clientSign) public onlyByVersionContractOrLogic {
